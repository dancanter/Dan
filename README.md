...
| **Guidance**                   | 111 searchable, individually-cited entries across four life phases — _During pregnancy_ (nutrition, supplements, food safety, exercise, sleep, wellbeing, weight, medications, vaccinations, existing conditions, alcohol & smoking, **work rights**, everyday safety, skincare, travel, infections), _Birth & labour_, _After birth_, and _Feeding_ |
| **Appointments**               | Your antenatal timeline, tailored to first vs subsequent pregnancy                                                                                                                                                                                                                                                                                   |
| **Get Help**                   | 13 urgent symptoms in plain language, each opening to _what to do now_ (one-tap call, read-aloud), _why it matters_, then reassurance where true                                                                                                                                                                                                     |
...
| **Myth or fact**               | The 16-card deck, one claim at a time. Guessing turns the card over; nothing is scored                                                                                                                                                                                                                                                               |
| **Can I eat it?**              | The food rules, sorted into four answers — _fine_, _cook it first_, _limit_, _avoid_                                                                                                                                                                                                                                                                 |
...
**"Since you were last here" tracks the week, and deliberately never the date.** Someone who opens the app once a fortnight should get something useful rather than the same screen again — you're in a new week, here's what became relevant while you were away. What it never says is how long it's been: _"you haven't been here in 9 days"_ is a streak wearing a different hat, aimed at exactly the wrong audience. A test asserts the banner never says _day_, _missed_, _behind_ or _streak_, and never appears on a first visit, an unchanged week, or while browsing ahead.
...
**No kick counting.** NHS and RCOG guidance is explicit that there's no target number and counting isn't recommended — so the app deliberately does _not_ ship a kick counter, and the myth deck says so directly. Same reasoning behind the home-doppler myth card.
...
**The three learning decks never tell anyone they're wrong.** Myth or fact, the food sort and the terms game all share one card component, and each consumes existing content by id — nothing medical is written in the screen itself. The design problem is that half these myths are things a relative said with total confidence, so being marked _incorrect_ for believing them attaches a small humiliation to something already anxious. There's no right and wrong: a guess turns the card over, and the response either agrees with you or says the claim _catches a lot of people_ — which is true, and is why the card exists. No score, no streak, no "4 of 16", because stopping after one is a complete use of it and a progress fraction would quietly say otherwise. A test asserts the words _wrong_, _incorrect_, _nope_ and _try again_ can never appear on any of the three.

**The food sort has four answers, not two.** _Cook it first_ and _avoid_ get collapsed together constantly, and that conflation is most of why the banned list feels far longer than it is — brie isn't banned, it needs heating. Restructuring prose into sortable items is a restatement of guidance, so it's checked rather than trusted: `validateContent()` asserts every item actually appears in the guide it points at. That check caught a real error on its first run — an item written as "tinned tuna" where the guidance says "limit tuna to 4 cans".

**"Need a minute?" routes a crisis away from itself before offering anything.** An app that cannot check whether you're well has no business running a wellbeing programme, so this is the narrowest possible version: two exercises, no mood tracking, no course, no streak. The order of the page _is_ the design — a breathing exercise is the wrong answer to a crisis, and offering one first would be worse than offering nothing, because it implies the app has understood the problem and thinks this will fix it. So the harder case is named at the top and sent to `/help/mental-health` before anything else appears. A test asserts that ordering. What's deliberately **absent** matters too: there's no 5-4-3-2-1 grounding exercise, useful as it is elsewhere, because the review this page rests on covered breathing, music, muscle relaxation, yoga and mindfulness — and grounding isn't among them. Including it would have meant either an uncited technique on a page whose whole claim is that everything is cited, or attributing it to an NHS page nobody here has read.
...
Routes are code-split, and Onboarding, Today and **Get Help** load eagerly — the daily entry point and the screen someone might need urgently should never wait on a chunk. First paint is **112kB gzipped of JavaScript** across 8 modules, down from 197kB across 20. The last of that was the content barrel: `src/content/index.ts` re-exports everything, so importing one symbol pulled all 111 guides and their bodies into the first paint. The eager screens now import the specific modules they need, and the weekly reading rules carry their guide's title rather than the whole guide — Today needed three titles and was downloading the entire library to get them. Duplicating those titles is only safe because `validateContent()` fails the build when one drifts from the guide it names.
...
**Nothing crashes to a blank page.** An error boundary wraps every route, and its fallback isn't a generic apology — it's the two things that matter when the app itself is broken: call 111, or call 999. Those are plain `tel:` anchors rather than routes, because routing is one of the things that might have failed. Same reasoning behind the bump gallery's IndexedDB handling: a browser that refuses to open a database (a private window, a managed device) gets told so explicitly, because an empty grid reads as _your photos are gone_.
...
npm run preview
npm test           # content integrity, component and accessibility tests (376)
npm run typecheck
npm run lint
...
© 2026 Dan Canter. All rights reserved.

The source code, written content and design of Field Notes are not open source. You can use the published app and read the code here, but you may not copy, modify, redistribute or reuse any of it without written permission. See [LICENSE](LICENSE) for the full terms, including the third-party material — NHS and other published guidance, open-source libraries and fonts — that belongs to its own owners.
