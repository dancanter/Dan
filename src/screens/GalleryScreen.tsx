...
export function GalleryScreen() {
  ...
  return (
    ...
    >
      ...
      <label
        htmlFor="bump-photo"
        className="flex min-h-[52px] w-full cursor-pointer items-center justify-center rounded-xl border border-moss bg-mossp px-4 text-read font-semibold text-mossd"
      >
        {busy
          ? 'Saving…'
          : currentWeek === null
      ...
      <p aria-live="polite" className="mt-2 text-small text-mossd">
        {photos.length === 0
          ? ''
                ...
                <>
                  ...
                  <div className="mt-2 flex items-center gap-3">
                    <button
                      ...
                      className="min-h-11 rounded-lg border border-line px-4 text-small font-semibold"
                    >
                      {playing ? 'Pause' : 'Play'}
                    </button>
                    <span className="font-mono text-fine text-soft">
            ...
            {photos.map((p) => (
              <li key={p.week} className="rounded-xl border border-line bg-card p-2">
                ...
                <div className="mt-1.5 flex items-center justify-between">
                  <span className="font-mono text-meta text-clay">Week {p.week}</span>
                  <button
                    type="button"
                    onClick={async () => {
