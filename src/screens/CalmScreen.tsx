  ...
  return (
    <Screen title="Need a minute?" lede={CALM_INTRO} width="reading">
      <div className="mb-6 rounded-xl border-2 border-alert bg-alertp px-4 py-3.5">
        <p className="m-0 text-body font-semibold leading-relaxed">{CALM_ESCALATION}</p>
        <p className="mb-3 mt-1.5 text-body leading-relaxed">
      ...
      {calmExercises.map((exercise) => (
        <section key={exercise.id} className="mb-8">
          ...
          <p className="mb-3 text-body leading-relaxed text-soft">{exercise.blurb}</p>
            ...
            <ol className="m-0 list-none p-0">
              {exercise.steps.map((step, i) => (
                <li key={step} className="mb-2 flex gap-3">
                  <span
                    aria-hidden="true"
                    className="mt-0.5 flex h-6 w-6 flex-none items-center justify-center rounded-full bg-mossp font-mono text-meta font-semibold text-mossd"
                  >
                    {i + 1}
                  </span>
      ...
      {calmFacts.map((fact) => (
        <div key={fact.text} className="mb-3 rounded-xl border border-line bg-card px-4 py-3.5">
          <p className="m-0 text-body leading-relaxed">{fact.text}</p>
