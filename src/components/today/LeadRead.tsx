...
export function LeadRead({ read, alreadyRead }: { read: WeekRead; alreadyRead: boolean }) {
  return (
    ...
    >
      ...
      <span className="mb-2 block font-display text-title font-semibold leading-snug text-ink">
        {read.title}
      </span>
      <span className="block text-body leading-relaxed text-soft">{read.why}</span>
      <span className="mt-2.5 block font-mono text-meta text-clay">Read this →</span>
    </Link>
  );
}
