...
export function WhatChanged({ previousWeek, currentWeek, excludeGuideId }: Props) {
  ...
  return (
    ...
    >
      ...
      {fresh.length > 0 && (
        <>
          <p className="mb-1.5 mt-2.5 text-body text-mossd">
            {fresh.length === 1
          ...
          <ul className="m-0 list-none p-0">
            {fresh.map(({ id, title, why }) => (
              <li key={id} className="mb-1">
                <Link
                  to={`/healthy?open=${id}`}
                  className="text-body font-semibold text-mossd underline"
                >
                  {title}
                </Link>
                <span className="block text-small italic text-soft">{why}</span>
              </li>
            ))}
          </ul>
        </>
