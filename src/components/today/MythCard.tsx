...
export function MythCard({ myth, reduceMotionOverride, onReveal }: Props) {
  ...
  return (
    <div className="rounded-xl border border-line bg-card p-4.5 px-4 py-4 text-center">
      ...
      <p className="mt-2 mb-3 font-display text-title font-semibold">“{myth.claim}”</p>

      {!revealed ? (
        <button
          type="button"
          onClick={reveal}
          className="min-h-11 w-full rounded-lg border-2 border-dashed border-line px-4 py-3 font-mono text-meta text-soft hover:border-moss hover:text-mossd"
        >
          Tap to reveal →
        </button>
      ) : (
        ...
        <div className={`border-t border-line pt-3 text-left ${reduce ? '' : 'reveal-in'}`}>
          ...
          <p className="mt-2 text-body">{myth.explanation}</p>
