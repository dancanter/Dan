...
export function MidwifeQuestionCard({ onSave }: Props) {
  ...
  return (
    <div className="rounded-xl border border-dashed border-moss bg-card p-4">
      <p className="m-0 mb-3 text-read italic">“{question}”</p>
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="min-h-11 rounded-lg border border-line bg-mossp px-3.5 text-small font-semibold text-mossd"
        >
          Another →
        </button>
        <button
          ...
          className="min-h-11 rounded-lg border border-line bg-clayp px-3.5 text-small font-semibold text-clay"
        >
          {saved ? 'Saved ✓' : 'Save this question'}
        </button>
      </div>
      <p aria-live="polite" className="mt-2 text-fine text-soft">
        {saved ? (
          <>
            Saved. It’s on{' '}
